...
export async function fetchListings(): Promise<ListingListItem[]> {
  const { data, error } = await supabase
    .from("listings")
    .select("id, title, status, price, currency, il, ilce")
...
export async function fetchListing(id: string): Promise<ListingInput> {
  const { data, error } = await supabase
    .from("listings")
    .select(
      "id, title, description, price, currency, size_m2, room_config, il, ilce, mahalle, latitude, longitude, phone, whatsapp, status, listing_images ( id, storage_key, position )",
    )
    .eq("id", id)
    .single();
